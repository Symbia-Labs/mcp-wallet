...
async fn main() {
    ...
    {
        let storage = Arc::new(
            EncryptedFileStorage::with_dir(storage_path.clone()).expect("Failed to create storage"),
        );
        ...
        let chat_tool = tools
            .iter()
            .find(|t| t.name == "openai_create_chat_completion")
            .unwrap();
        let props = chat_tool.input_schema.properties.as_ref();
        println!(
            "BEFORE SAVE - properties count: {}",
            props.map(|p| p.len()).unwrap_or(0)
        );
        println!(
            "BEFORE SAVE - has model: {}",
            props.map(|p| p.contains_key("model")).unwrap_or(false)
        );
        println!(
            "BEFORE SAVE - has messages: {}",
            props.map(|p| p.contains_key("messages")).unwrap_or(false)
        );
        ...
        match stored {
            Some(s) => {
                ...
                let op = s
                    .operations
                    .iter()
                    .find(|o| o.operation_id == "createChatCompletion");
                match op {
                    Some(op) => {
                ...
                match chat_tool {
                    Some(tool) => {
                        ...
                        println!(
                            "\nAFTER LOAD - properties count: {}",
                            props.map(|p| p.len()).unwrap_or(0)
                        );
                        println!(
                            "AFTER LOAD - has model: {}",
                            props.map(|p| p.contains_key("model")).unwrap_or(false)
                        );
                        println!(
                            "AFTER LOAD - has messages: {}",
                            props.map(|p| p.contains_key("messages")).unwrap_or(false)
                        );
