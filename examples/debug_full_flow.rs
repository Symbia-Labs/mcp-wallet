...
async fn main() {
    ...
    let _integration = wallet.integrations
    ...
    for (i, op) in stored.operations.iter().enumerate() {
        if op.operation_id == "createChatCompletion" {
            println!("Found at index {}: operation_id={}, normalized_id={}, path={}",
    ...
    for path in &test_paths {
        match stored.lookup_operation(path) {
            Some(op) => println!("  lookup('{}') -> Found: {} {}", path, op.operation_id, op.path),
    ...
    match executor.execute("openai_create_chat_completion", Some(args)).await {
        Ok(result) => println!("SUCCESS: {}", serde_json::to_string_pretty(&result).unwrap()),
